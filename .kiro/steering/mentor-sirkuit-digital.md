...
- **Integritas Struktur DOM Mutlak:** Saat melakukan modifikasi kelas CSS, atribut, atau tata letak pada elemen HTML, AI Agent dilarang keras memotong atau menghilangkan tag pembuka maupun penutup dari elemen yang saling terikat secara logika (seperti pasangan label for dan input id). 
- **Spesifikasi Blok Jangkar (Contextual Patching):** Untuk mencegah tabrakan penggabungan kode (diff merge collision) pada struktur HTML yang repetitif, AI Agent wajib menyertakan minimal 3 baris elemen induk atau kontainer pembungkus di atas dan di bawah baris yang dimodifikasi sebagai jangkar konteks yang kaku. Jika struktur terlalu rapat, cetak seluruh blok kontainer grup kontrol tersebut secara utuh dari awal hingga akhir.
...
- **Monomorphic Array Preservation:** Dilarang menggunakan fungsi `splice()`, `push()`, atau `pop()` pada array komponen sirkuit selama simulasi berjalan. Manipulasi status dicabut atau rusak wajib dilakukan murni melalui mutasi nilai properti flag boolean internal objek (`isDetached = true/false`) untuk mencegah deoptimisasi Hidden Class pada mesin V8.
- **Canvas Coordinates Hit Boxing:** Interaksi mencabut lampu secara langsung pada Canvas wajib menggunakan fungsi `getBoundingClientRect()` untuk mengalkulasi rasio koordinat sentuh secara relatif terhadap kerapatan piksel fisik layar perangkat, guna mencegah pergeseran titik hitbox pada orientasi landscape seluler perangkat.
